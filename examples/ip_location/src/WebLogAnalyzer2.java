...
/**
 * <p>This program analyzes apache web log entries based on their location.</p> <p>Hadoop is supposed to have built in
 * functionality to get data from two sources and to join data based on join keys, but I could not get this to work.
 * Instead a temporary collection is used to join the data.  The location information is stored back in the original
 * input collection for future use.</p>
 *
...
public class WebLogAnalyzer2 extends MongoTool {
    ...
    public static class IpRangeMapper
        ...
        protected void map( LongWritable key, Text value, Context context ) throws IOException, InterruptedException{
            ...
            String range =
                    IpCsvParser.getIpRange( map1.get( IpCsvParser.Key.IP_FROM ), map1.get( IpCsvParser.Key.IP_TO ) );
            rangeText.set( range );
    ...
    public static class IpRangeReducer extends Reducer<Text, BSONWritable, Text, BSONObject> {
        ...
        @Override
        protected void reduce( Text key, Iterable<BSONWritable> values, Context context )
    ...
    public static class TheWeglogEntryMapper
        ...
        protected void map( Object key, BSONObject value, Context context ) throws IOException, InterruptedException{
            ...
            context.write( rangeText,
                           new BSONWritable( new BasicBSONObject( "weblog_id", (org.bson.types.ObjectId) key ) ) );
    ...
    public static class TheWeblogEntryReducer extends Reducer<Text, BSONWritable, String, BSONObject> {
        ...
        public void reduce( Text key, Iterable<BSONWritable> values, Context context )
            ...
            final BasicDBObject val =
                    new BasicDBObject().append( "$pushAll", new BasicDBObject().append( "weblogids", list ) );
    ...
    public static class TheWeglogEntryUpdateMapper
        ...
        protected void map( Object key, BSONObject value, Context context ) throws IOException, InterruptedException{
            ...
            for ( Object weblog_id : weblogIdList ){
                ...
                context.write( new BSONWritable( id_wrapper ),
                               new BSONWritable( (BSONObject) value.get( "iprange" ) ) );
            }
        }
    }
    ...
    public static class TheWeglogEntryUpdateReducer extends Reducer<BSONWritable, BSONWritable, String, BSONObject> {
        @Override
        protected void reduce( BSONWritable wrapped_id, Iterable<BSONWritable> values, Context context )
                throws IOException, InterruptedException{
            ...
            final BasicDBObject val =
                    new BasicDBObject().append( "$set", new BasicDBObject( IP_RANGE_FIELD_NAME, value ) );
            final BasicDBObject wrapper = new BasicDBObject();
    ...
    static void shardCollection( com.mongodb.MongoURI uri ){
        ...
        try {
            ...
            BasicDBObject command =
                    new BasicDBObject( "shardcollection", uri.getDatabase() + "." + uri.getCollection() ).
                                                                                                                 append( "key",
                                                                                                                         new BasicDBObject(
                                                                                                                                 "_id",
                                                                                                                                 1 ) );
