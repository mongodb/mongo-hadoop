...
def generate_jar_name(prefix, version_suffix):
    return prefix + "-" + version_suffix + "-hadoop_" + HADOOP_RELEASE + ".jar"

treasury_jar_name = generate_jar_name("treasury_yield", VERSION_SUFFIX)
logging.error("treasury_jar_name = " + treasury_jar_name)
...
JOBJAR_PATH=os.path.join(MONGO_HADOOP_ROOT, "examples", "treasury_yield", "build/libs", treasury_jar_name) 
JSONFILE_PATH=os.path.join(MONGO_HADOOP_ROOT, 'examples', 'treasury_yield', 'src', 'main', 'resources', 'yield_historical_in.json');
STREAMING_JARPATH=os.path.join(MONGO_HADOOP_ROOT, "streaming", "build/libs", streaming_jar_name)
STREAMING_MAPPERPATH=os.path.join(MONGO_HADOOP_ROOT, "streaming", "examples", "treasury", "mapper.py")

STREAMING_REDUCERPATH=os.path.join(MONGO_HADOOP_ROOT, "streaming", "examples", "treasury", "reducer.py")
